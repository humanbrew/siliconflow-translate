...
1. 在VS Code中打开本项目文件夹
2. 确保已安装 **Comment Translate** 插件（这是必需的依赖）
3. 按 `F5` 或点击"运行和调试"面板中的"Run Extension"
4. 会打开一个新的VS Code窗口（Extension Development Host）

#### 步骤4：在新窗口中配置和使用

在新打开的VS Code窗口中：

1. **安装Comment Translate插件**（如果还没有安装）
   - 在新窗口中按 `Ctrl+Shift+X`（Mac: `Cmd+Shift+X`）打开扩展市场
   - 搜索并安装 `Comment Translate`

2. **配置翻译源**
   - 按 `Ctrl+Shift+P`（Mac: `Cmd+Shift+P`）打开命令面板
   - 输入并选择 `Comment Translate: Change translation source`
   - 选择 `SiliconFlow Translate`

3. **配置API密钥**
   - 按 `Ctrl+,`（Mac: `Cmd+,`）打开设置
   - 搜索 `siliconflowTranslate`
   - 配置以下设置：
     - `SiliconFlow Translate: Api Key`: 输入您的API密钥
     - `SiliconFlow Translate: Model`: 选择模型（如 `Qwen/Qwen2.5-7B-Instruct`）

4. **测试翻译功能**
   - 打开任意代码文件
   - 将鼠标悬停在注释上，应该能看到翻译结果
   - 或者选中文本，使用Comment Translate的翻译命令

#### 调试技巧

- **热重载**：修改代码后，在调试控制台按 `Ctrl+R`（Mac: `Cmd+R`）重新加载扩展
- **查看日志**：在调试控制台（Debug Console）中查看输出和错误信息
- **断点调试**：在代码中设置断点，可以单步调试

#### 常见问题

**Q: 新窗口中找不到 "SiliconFlow Translate" 选项？**
- 确保已编译项目（`npm run compile`）
- 检查调试控制台是否有错误信息
- 尝试重新加载窗口（`Ctrl+R` 或 `Cmd+R`）
